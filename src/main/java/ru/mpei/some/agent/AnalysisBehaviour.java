    ...
    public void action() {
        ...
        if (message != null){
            ...
            if (agentNear.getInfoAgent().size() == 1) {
                myAgent.addBehaviour(new SendResultBehaviour("fail!-!" + message.getContent(), agents[agents.length - 3]));
            } else {
                if (myAgent.getLocalName().equals(agents[agents.length-2])){
                    myAgent.addBehaviour(new SendResultBehaviour(message.getContent(), agents[agents.length-3]));
                }else{
                    for(InfoAgent infoAgent: agentNear.getInfoAgent()){
                        if (!infoAgent.getLocalName().equals(message.getSender().getLocalName())){
                            if (!(Arrays.asList(agents).contains(infoAgent.getLocalName())
                                    && !infoAgent.getLocalName().equals(agents[agents.length-2]))){
